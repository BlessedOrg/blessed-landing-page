...
const pricingPlans = [
  {
    ...
    price: "$0",
    description:
      "👋 We waived all fees for you. Perfect for small, mid or one-timer events. We'll rate limit though..",
...
const PricingPlan = ({ plan }) => (
  <div
    className={`flex flex-col items-start gap-4 p-6 md:p-8 w-full md:w-[296px] border-2 ${plan.borderColor} bg-[#FFF] rounded-[24px] shadow-[0px_1px_10px_0px_rgba(0,0,0,0.06)] z-10 mb-6 md:mb-0`}
  >
    ...
    {plan.subDescription && (
      <p className="text-sm md:text-[16px] text-left">{plan.subDescription}</p>
    )}
    ...
    {plan.action.type === "modal" ? (
      ...
      <Link href={plan.action.props.href}>
        <Button
          variant="outline"
          size="default"
          className={plan.action.props.className}
        >
          {plan.action.label}
        </Button>
      </Link>
...
export const PricingHeader = () => {
  ...
  return (
    <header
      ref={headerRef}
      className="relative flex flex-col gap-6 md:gap-8 items-center py-8 md:py-10 px-4 bg-cover bg-center w-full overflow-hidden"
    >
