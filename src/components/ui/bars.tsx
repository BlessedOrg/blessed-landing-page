...
export const Bars = ({ barHeight = 75, isInView = true, color = "bg-green-500", rightBars, leftBars, zIndex = -1 }: IBars) => {
  const values = {
    leftBars: {
      amount: leftBars?.amount ?? 3,
      widthPercentage: leftBars?.widthPercentage || 60,
      show: leftBars?.show ?? false
    },
    rightBars: {
      amount: rightBars?.amount ?? 4,
      widthPercentage: rightBars?.widthPercentage || 25,
  ...
  const rightBarsGenerate = Array.from({ length: values.rightBars.amount }, (_, index) => {
